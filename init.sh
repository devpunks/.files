...
pkg install pkg-config # https://en.m.wikipedia.org/wiki/Pkg-config
...
pkg install openssl openssl-tool # https://openssl.org
...
export PATH=$PATH # Must set PATH to prevent Nil split error
...
pkg install libvips --no-install-recommends --fix-missing # https://github.com/libvips/libvips
pkg install ffmpeg
